  ...
  <v-container style="max-height: 500px" class="overflow-y-auto">
    ...
    <v-slider :thumb-size="20" min="35" max="45" ticks="always" thumb-label="always">
      <template v-slot:prepend>
        <img width="30px" :src="'/covid19-screening-assistant/symptoms/1.svg'" alt />
      </template>
    </v-slider>
    <div v-for="(question, index) in questions" :key="index">
      ...
      <v-slider :tick-labels="seasons" min="0" max="3" ticks="always">
        <template v-slot:prepend>
          <img
            width="30px"
            :src="'/covid19-screening-assistant/symptoms/' + (index + 2) + '.svg'"
            alt
          />
        </template>
        ...
        <template v-slot:thumb-label="props">
          ...
          <img
            width="30px"
            :src="'/covid19-screening-assistant/feelings/' + season(props.value)"
            alt
          />
        </template>
      </v-slider>
